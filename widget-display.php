<?php
	...
	**/      
	  ...
	  */  
		...
		$taxonomy_field = isset( $instance['taxonomy_field'] ) ? esc_attr($instance['taxonomy_field']) : 'slug';
        ...
        if ( $widget_query->have_posts() ) : while ( $widget_query->have_posts() ) : $widget_query->the_post();
            ...
            if ( $show_thumb == true ) {
                ...
                the_post_thumbnail('thumbnail', array('class' => 'hd-thumb-image'));
				echo '</div>';
            }
			if ( $show_title == true ) {
			...
			if ( $show_taxonomy == true || $show_date == true || $show_author == true ) {
				...
				if ( $show_taxonomy == true ) {
	            	echo '<p class="hd-taxonomy">' . get_the_term_list( get_the_ID(), $instance['taxonomy'], '', ', ' ). '</p>';
				}
				echo '</div>';
			}
            ...
            if ( $show_read_more == true ) {
                echo '<a class="hd-read-more-button btn btn-default" href="' . get_permalink() . '" title="'.get_the_title(). '">'. $instance['read_more_text'] .'</a>';
            }
            echo '</li>';
